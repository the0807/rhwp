...
fn print_usage() {
    ...
    println!("  옵션: --triplet-matrix  # Stage 14 PARA_HEADER/PARA_TEXT/PARA_CHAR_SHAPE 조합 probe");
}
...
fn build_probe_file(
    ...
    let (patched_section, counts) = patch_first_para_section(&oracle_section, &generated_section, kind)?;
...
fn patch_first_para_section(
    ...
    match kind {
        ...
        ProbeKind::OracleHeaderSubtree => {
            ...
            .ok_or_else(|| "generated first paragraph Header subtree를 찾지 못했습니다".to_string())?;
        ...
        | ProbeKind::OracleTopLevelCtrlHeaders => {
            ...
            Ok((apply_record_patches(generated_raw, &generated_records, patches), counts))
        }
    }
}
...
fn patch_matching_first_tag(
    ...
    let oracle_index = find_first_para_record(oracle_records, oracle_first, tag)
        .ok_or_else(|| format!("oracle first paragraph에서 {} record를 찾지 못했습니다", tags::tag_name(tag)))?;
...
fn replace_record_range(
    ...
    let mut out = Vec::with_capacity(
        generated_raw.len() - generated_bytes.len() + oracle_bytes.len(),
    );
...
fn render_para_text_markers(
    ...
    let _ = writeln!(report);
    let oracle_payload = first_payload(oracle_raw, oracle_records, oracle_first, tags::HWPTAG_PARA_TEXT);
    let generated_payload =
        first_payload(generated_raw, generated_records, generated_first, tags::HWPTAG_PARA_TEXT);
...
fn render_char_shape_summary(
    ...
    let _ = writeln!(report);
    let oracle_payload =
        first_payload(oracle_raw, oracle_records, oracle_first, tags::HWPTAG_PARA_CHAR_SHAPE);
    ...
    let _ = writeln!(report, "- oracle: `{}`", format_char_shape_entries(oracle_payload));
    let _ = writeln!(
        report,
        "- generated: `{}`",
...
fn render_top_level_ctrl_sequence(
    ...
    let _ = writeln!(report);
    let _ = writeln!(report, "- oracle: `{}`", format_ctrl_sequence(oracle_raw, oracle_records, oracle_first));
    let _ = writeln!(
        report,
        "- generated: `{}`",
...
fn read_required_section(
    ...
    read_decompressed_section(bytes, section, compressed)?.ok_or_else(|| {
        format!("{label} BodyText/Section{section} 스트림을 읽지 못했습니다")
    })
}
